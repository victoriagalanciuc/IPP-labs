...
class GameView: UIView {
  ...
  func sizeAvailableForShapes() -> CGSize {
    ...
    let topY = CGRect(x: x, y: y, width: width, height: height).maxY + padding
  ...
  private final func attributedTextForScore(aScore: Int) -> NSAttributedString? {
    return NSAttributedString(string: "Score: \(aScore)",
      attributes: [ NSFontAttributeName : UIFont.boldSystemFont(ofSize: 32),
        NSForegroundColorAttributeName : aScore < 0 ? UIColor.red : UIColor.black])
  }
